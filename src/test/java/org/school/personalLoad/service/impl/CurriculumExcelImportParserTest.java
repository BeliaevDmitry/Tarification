...
class CurriculumExcelImportParserTest {
    ...
    @Test
    void doesNotTreatDirectionOrTeacherRowsAsClassHeader() {
        try (XSSFWorkbook workbook = baseWorkbook("ООО")) {
            Sheet sheet = workbook.getSheetAt(0);
            row(sheet, 0, "Учебный год 2026-2027");
            row(sheet, 1, "Период обучения", "", "Год");
            row(sheet, 2, "Направленность класса", "", "Классический профиль");
            row(sheet, 3, "ФИО классного руководителя", "", "Петров П.П.");
            row(sheet, 4, "Класс", "", "8-Б");
            row(sheet, 5, "Обязательная часть");
            row(sheet, 6, "", "География", "1");

            List<CurriculumExcelImportParser.ImportedCurriculumRow> rows = parser.parse(workbook);
            assertEquals(1, rows.size());
            assertEquals("8-Б", rows.get(0).className());
            assertEquals("Классический профиль", rows.get(0).classDirection());
            assertEquals("Петров П.П.", rows.get(0).classTeacher());
        } catch (Exception e) {
            fail(e);
        }
    }
