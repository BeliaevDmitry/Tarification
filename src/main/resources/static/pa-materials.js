...
function publicDownload(row,file){return file.legacy?`/api/public/pa/materials/${row.id}/download?kind=${file.kind}`:`/api/public/pa/materials/files/${file.id}/download`;}
function publicFileLinks(row,files,label,primary=false){return files.map((file,index)=>`<a${primary?' class="primary"':''} href="${publicDownload(row,file)}">${publicHtml(label)}${files.length>1?` ${index+1}`:''}: ${publicHtml(file.fileName||'скачать')}</a>`).join('');}
...
function renderPublicMaterials(){
    ...
    document.getElementById('public-material-list').innerHTML=rows.length?rows.map(row=>{const texts=row.textFiles||[];const answers=row.answerFiles||[];return `<article class="public-material"><h3>${publicHtml(row.subjectName)} — ${row.scopeType==='CLASS'?'класс':'параллель'} ${publicHtml(row.scopeValue)}</h3><div class="public-material-meta"><span>${publicHtml(publicWorkRu(row.workType))}</span><span>•</span><span>${publicHtml(publicLevelRu(row.level))}</span><span>•</span><span>текстов: ${texts.length}, ответов: ${answers.length}</span></div><div class="public-material-actions">${texts.length?publicFileLinks(row,texts,'Текст работы',true):'<span class="public-material-missing">Тексты не загружены</span>'}${answers.length?publicFileLinks(row,answers,'Ответы'):'<span class="public-material-missing">Ответы не загружены</span>'}</div></article>`;}).join(''):'<p class="muted">По выбранным условиям файлов нет.</p>';
}
