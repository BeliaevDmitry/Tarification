...
async function reload() {
    const [rowsResult, buildingsResult, teacherResult] = await Promise.allSettled([
    ...
    if (rowsResult.status !== "fulfilled") {
        print({ warning: "Не удалось загрузить сохранённые классы", details: rowsResult.reason?.message || String(rowsResult.reason || "") });
    }
    teachers = (teacherRows || []).map((r) => norm(r.fioTeacher)).filter(Boolean);
    classRows = actualRows;
