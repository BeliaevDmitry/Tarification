...
public class TeacherNotificationsController {
    ...
    private byte[] generateDoc(String fio, String year, LocalDate d) throws Exception {
        ...
        String director = appUserRepository.findAllByRole(UserRole.DIRECTOR).stream().findFirst().map(u -> u.getFullName()).orElse("Директор");
        try (XWPFDocument doc = new XWPFDocument(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            XWPFParagraph p = doc.createParagraph(); p.createRun().setText("Уведомление о нагрузке");
            doc.createParagraph().createRun().setText("Дата: " + d);
            doc.createParagraph().createRun().setText("Педагог: " + fio);
            doc.createParagraph().createRun().setText("Директор: " + director + " (school=" + System.getenv().getOrDefault("SCHOOL_CODE", "demo") + ")");
