...
public interface PaReportVersionRepository extends JpaRepository<PaReportVersion, Long> {
    ...
    List<PaReportVersion> findTop10ByAcademicYearAndSubjectCatalogIdAndScopeTypeAndSchoolClassIdAndLevelAndWorkTypeAndWorkDateOrderByCreatedAtDesc(
            String academicYear,
            Long subjectCatalogId,
    ...
    List<PaReportVersion> findAllByAcademicYearAndSubjectCatalogIdAndScopeTypeAndSchoolClassIdAndLevelAndWorkTypeAndWorkDate(
            String academicYear,
            Long subjectCatalogId,
            PaScopeType scopeType,
            Long schoolClassId,
    ...
    @Query("select coalesce(max(r.versionNo), 0) from PaReportVersion r where r.academicYear = :academicYear and r.subjectCatalogId = :subjectCatalogId and r.scopeType = :scopeType and r.schoolClassId = :schoolClassId and r.level = :level and r.workType = :workType and ((r.workDate is null and :workDate is null) or r.workDate = :workDate)")
