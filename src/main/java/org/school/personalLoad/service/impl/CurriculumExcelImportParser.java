...
class CurriculumExcelImportParser {
    ...
    private HeaderMarkers detectMarkers(Sheet sheet) {
        ...
        for (int i = 0; i <= Math.min(sheet.getLastRowNum(), 40); i++) {
            ...
            for (Cell cell : row) {
                String value = normalize(readCell(cell)).toLowerCase(Locale.ROOT).replace("ё", "е");
                if (value.contains("период обучения")) {
                    ...
                    teacherRow = i;
                } else if (isClassHeaderMarker(value)) {
                    classRow = i;
    ...
    private boolean isClassHeaderMarker(String value) {
        return value.equals("класс")
                || value.equals("класс/группа")
                || value.equals("класс / группа")
                || value.startsWith("класс/");
    }
